...
class LiDOSTuner:
    def __init__(self, system, workloads, run_id, optimization_goal, initial_seeds, common_seeds, max_generation,
        ...
        self.prev_optimized_pop = None  # 用于存储上一个工作负载的优化种群
    ...
    def run(self):
        ...
        for workload_id, workload_name in enumerate(self.workloads):
            print(f"\n[LiDOS | RUN {self.run_id}] >>> Workload {workload_id + 1}/{len(self.workloads)}: {workload_name}")
            ...
            if workload_id == 0:
                ...
                init_population = [cfg for cfg, _ in self.prev_optimized_pop]
