...
class DLiSAB7Tuner:
    def __init__(self, system, workloads, run_id, optimization_goal, initial_seeds, common_seeds, max_generation, pop_size, fallback=None):
        ...
        self.similarity_log_path = f"results/run{self.run_id}/DLiSA-B7/{self.system}/similarity.json"
    ...
    def run(self):
        ...
        for workload_id, workload_name in enumerate(self.workloads):
            print(f"\n[DLiSA-B7 | RUN {self.run_id}] >>> Workload {workload_id + 1}/{len(self.workloads)}: {workload_name}")
    ...
    def generate_seeds_by_dlisab(self, current_workload):
        ...
        if len(self.history_optimized_population) == 1:
            ...
            similar_workloads = self.rank_workload_similarity_analysis(current_workload, beta=0.7)
    ...
    def weighted_config_seeding(self, similar_workloads: dict, current_workload):
        ...
        output_dir = f"results/run{self.run_id}/DLiSA-B7/{self.system}/{current_workload}"
