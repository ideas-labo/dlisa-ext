...
class DLiSABITuner:
    def __init__(self, system, workloads, run_id, optimization_goal, initial_seeds, common_seeds, max_generation, pop_size, fallback=None):
        ...
        self.similarity_log_path = f"results/run{self.run_id}/DLiSA-BI/{self.system}/similarity.json"
    ...
    def run(self):
        ...
        for workload_id, workload_name in enumerate(self.workloads):
            print(f"\n[DLiSA-BI | RUN {self.run_id}] >>> Workload {workload_id + 1}/{len(self.workloads)}: {workload_name}")
    ...
    def generate_seeds_by_dlisab(self, current_workload):
        ...
        if len(self.history_optimized_population) == 1:
            ...
            similar_workloads = self.rank_workload_similarity_analysis(current_workload, beta=0.4)
            ...
            if not similar_workloads:
                ...
                # 将weighted_config_seeding 改成随机 seeding
                init_population = self.random_config_seeding(similar_workloads)
    ...
    def weighted_config_seeding(self, similar_workloads: dict, current_workload):
        ...
        for wl in similar_workloads:
            ...
            for cfg, _ in top_k:
                ...
                cfg_to_sim[cfg_t].append(similar_workloads[wl])
        ...
        output_dir = f"results/run{self.run_id}/DLiSA-BI/{self.system}/{current_workload}"
    ...
    def random_config_seeding(self, similar_workloads: dict):
        """
        消融版本的种子生成方法：
        从所有相似 workload 的历史种群中，随机选择 pop_size 个个体作为初始化种群。
        不使用 top-k，不使用加权，仅用随机。
        """
        candidates = set()
        ...
        for wl in similar_workloads:
            pop = self.history_optimized_population[wl]
            for cfg, _ in pop:
                candidates.add(tuple(cfg))  # 去重

        candidates = list(candidates)

        selected = []
        if len(candidates) >= self.pop_size:
            selected_cfgs = random.sample(candidates, self.pop_size)
            selected = [list(cfg) for cfg in selected_cfgs]
        else:
            selected = [list(cfg) for cfg in candidates]
