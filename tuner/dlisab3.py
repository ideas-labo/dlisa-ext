...
class DLiSAB3Tuner:
    def __init__(self, system, workloads, run_id, optimization_goal, initial_seeds, common_seeds, max_generation, pop_size, fallback=None):
        ...
        self.similarity_log_path = f"results/run{self.run_id}/DLiSA-B3/{self.system}/similarity.json"
    ...
    def run(self):
        ...
        for workload_id, workload_name in enumerate(self.workloads):
            print(f"\n[DLiSA-B3 | RUN {self.run_id}] >>> Workload {workload_id + 1}/{len(self.workloads)}: {workload_name}")
