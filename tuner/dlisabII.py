...
class DLiSABIITuner:
    def __init__(self, system, workloads, run_id, optimization_goal, initial_seeds, common_seeds, max_generation, pop_size, fallback=None):
        ...
        self.similarity_log_path = f"results/run{self.run_id}/DLiSA-BII/{self.system}/similarity.json"
    ...
    def run(self):
        ...
        for workload_id, workload_name in enumerate(self.workloads):
            print(f"\n[DLiSA-BII | RUN {self.run_id}] >>> Workload {workload_id + 1}/{len(self.workloads)}: {workload_name}")
    ...
    def generate_seeds_by_dlisab(self, current_workload):
        ...
        if len(self.history_optimized_population) == 1:
            ...
            # 将rank_workload_similarity_analysis 改成随机触发seeding，触发时对所有的历史workload进行weighted;
            if random.random() < 0.5:
                init_population = self.config_manager.random_initialize_population(self.pop_size)
            else:
                # 用所有历史 workload，统一赋相似度为 1.0
                all_history_workloads = list(self.history_optimized_population.keys())
                fake_similar_workloads = {wl: 1.0 for wl in all_history_workloads}
                init_population = self.weighted_config_seeding(fake_similar_workloads, current_workload)
    ...
    def weighted_config_seeding(self, similar_workloads: dict, current_workload):
        ...
        output_dir = f"results/run{self.run_id}/DLiSA-BII/{self.system}/{current_workload}"
