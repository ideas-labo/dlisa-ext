...
def collect_data(systems, algorithms, runs, base_path='../results'):
        ...
        for workload in workloads:
            ...
            if (algo_df.nunique() <= 1).any():
                ...
                try:
                    ...
                    print('Scott-Knott ESD failed:', e)
    ...
    all_df.to_csv('RQ4/scott_knott.csv', index=False)


def main():
    
    systems = ['batik', 'dconvert', 'h2', 'jump3r', 'kanzi', 'lrzip', 'x264', 'xz', 'z3']
    compared_algorithms = ['DLiSA-B0', 'DLiSA-B1', 'DLiSA-B2', 'DLiSA-B3', 'DLiSA-B4', 'DLiSA-B', 'DLiSA-B6', 'DLiSA-B7', 'DLiSA-B8', 'DLiSA-B9']
    runs = 100
    collect_data(systems, compared_algorithms, runs)

    df = pd.read_csv('RQ4/scott_knott.csv')
    ...
    for system in systems:
        system_df = pd.read_csv(f'RQ4/{system}.csv')
